...
void franka_pole::PoleState::update(const ros::Time &time)
{
    ...
    angle(0) = -atan2(up(1), up(2));
    angle(1) = atan2(up(0), up(2));

    //Noise
    joint_angle(0) += _random_angle_distributions[0](_random_engine);
    angle(0) += _random_angle_distributions[0](_random_engine);
    if (_model == Model::D2 || _model == Model::D2b)
    {
        joint_angle(1) += _random_angle_distributions[1](_random_engine);
        angle(1) += _random_angle_distributions[1](_random_engine);
    }

    //Differentiation & update
    _joint_dangle = (joint_angle - _joint_angle) / (time.toSec() - _timestamp);
    _dangle = (angle - _angle) / (time.toSec() - _timestamp);
    _angle = angle;
    ...
    */
    ...
    _controller->publisher->set_pole_angle(_dangle);
